...
p2 <- ggplot(d, aes(x=pvalue, group=known)) + geom_density(fill = NA, position="dodge", aes(colour=known)) + scale + xlab("q-value of protein-side effect association")+opts(aspect.ratio=2) + facet_grid(.~dataset)
